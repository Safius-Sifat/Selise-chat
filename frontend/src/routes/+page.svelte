<script lang="ts">
	...
	const buildSocketUrl = (): string => {
		...
		const targetNickname = nickname.trim() || `user-${Math.random().toString(36).slice(2, 6)}`;
	...
	const addSystemMessage = async (text: string, group?: string): Promise<void> => {
		messages = [
			...messages,
			{
	...
	const addChatMessage = async (
		payload: Required<Pick<ChatMessage, 'from' | 'text' | 'group' | 'at' | 'mine'>>
	): Promise<void> => {
		messages = [
			...messages,
			{
	...
	const connect = (): void => {
		...
		ws.addEventListener('message', async (event) => {
			...
			if (payload.type === 'connected') {
				...
				if (activeTab === 'private' && typeof payload.group === 'string') {
			...
			if (activeTab === 'private' && payload.type === 'joined') {
				...
				await addSystemMessage(`You switched to #${activeRoom || roomInput || 'default'}`, activeRoom);
				return;
			}
			...
			if (activeTab === 'private' && payload.type === 'system' && typeof payload.message === 'string') {
				await addSystemMessage(payload.message, payload.group ?? activeRoom);
				return;
			}

			if (activeTab === 'private' && payload.type === 'group-broadcast' && typeof payload.message === 'string') {
				...
				await addChatMessage({
					...
					mine: from === currentClientId
				});
				return;
			}

			if (activeTab === 'public' && payload.type === 'broadcast' && typeof payload.message === 'string') {
				const from = typeof payload.from === 'string' ? payload.from : 'unknown';
				await addChatMessage({
					from,
					text: payload.message,
		...
		ws.addEventListener('error', async () => {
			...
			await addSystemMessage('Could not connect to WebSocket server', activeChannelLabel());
		});
	};
...
<main class="min-h-screen bg-base-200 p-3 md:p-6">
	<div class="mx-auto flex max-w-6xl flex-col gap-4">
		<div class="navbar rounded-box border border-base-300 bg-base-100 px-4 shadow-sm">
			<div class="flex-1">
				<h1 class="text-lg font-semibold md:text-xl">Chat Hub</h1>
				<p class="text-xs text-base-content/60">Secure domain: selise.notice.fit (SSL)</p>
			</div>
			...
			<div class="flex items-center gap-2">
				<span class="badge badge-outline">Channel: {activeChannelLabel()}</span>
				<span class={`badge ${status === 'connected' ? 'badge-success' : status === 'connecting' ? 'badge-warning' : 'badge-ghost'}`}>
					{statusLabel}
				</span>
			</div>
		</div>
		...
		<div class="grid min-h-[80vh] gap-4 lg:grid-cols-[320px_1fr]">
			<aside class="card border border-base-300 bg-base-100 shadow-sm">
				<div class="card-body gap-4">
					<h2 class="card-title text-base">{activeTab === 'public' ? 'Public Channel' : 'Private Room'}</h2>

					<label class="form-control w-full gap-1">
						<span class="label-text text-sm">Secure WebSocket Server</span>
						<input class="input input-bordered w-full" bind:value={wsBaseUrl} placeholder="wss://selise.notice.fit" />
					</label>

					<label class="form-control w-full gap-1">
						<span class="label-text text-sm">Your Name</span>
						<input class="input input-bordered w-full" bind:value={nickname} placeholder="safiu" />
					</label>

					{#if activeTab === 'private'}
						<label class="form-control w-full gap-1">
							<span class="label-text text-sm">Group Name</span>
							<input class="input input-bordered w-full" bind:value={roomInput} placeholder="general" />
						</label>
					{:else}
						<p class="rounded-box bg-base-200 p-3 text-sm text-base-content/80">
							Public tab broadcasts messages to everyone connected to the public channel.
						</p>
					{/if}

					<div class="grid grid-cols-2 gap-2">
						<button class="btn btn-primary" on:click={joinOrSwitchRoom}>
							{#if activeTab === 'public'}
								{status === 'connected' ? 'Reconnect Public' : 'Join Public'}
							{:else}
								{status === 'connected' ? 'Switch Room' : 'Join Room'}
							{/if}
						</button>
						<button class="btn btn-outline" on:click={disconnect} disabled={status !== 'connected'}>Leave</button>
					</div>
					...
					{#if activeTab === 'private'}
						...
						<div>
							<div class="mb-2 flex items-center justify-between">
								...
								<span class="badge badge-neutral badge-sm">{members.length}</span>
							</div>

							{#if members.length === 0}
								<p class="text-sm text-base-content/70">No members online.</p>
							{:else}
								<ul class="menu rounded-box max-h-72 gap-1 overflow-y-auto bg-base-200 p-2">
			...
			<section class="card border border-base-300 bg-base-100 shadow-sm">
				<div class="card-body h-full p-0">
					...
					<div bind:this={messageContainer} class="flex-1 space-y-2 overflow-y-auto px-3 py-3 md:px-4">
						{#if messages.length === 0}
							<div class="hero min-h-[320px] rounded-box bg-base-200">
							...
							{#each messages as item}
								{#if item.kind === 'system'}
									<div class="flex justify-center">
										<span class="badge badge-ghost badge-sm max-w-full truncate">{item.text}</span>
									</div>
								{:else}
									<div class={`chat ${item.mine ? 'chat-end' : 'chat-start'}`}>
										<div class="chat-header">
											{item.from}
											<time class="text-xs opacity-60">{formatTime(item.at)}</time>
										</div>
										<div class={`chat-bubble max-w-full whitespace-pre-wrap break-all ${item.mine ? 'chat-bubble-primary' : 'chat-bubble-neutral'}`}>
											{item.text}
										</div>
									</div>
								{/if}
					...
					<div class="border-t border-base-300 p-3 md:p-4">
						<div class="join w-full">
							...
							/>
							<button class="btn btn-primary join-item" on:click={sendChatMessage} disabled={status !== 'connected'}>
								Send
							</button>
						</div>
					</div>
