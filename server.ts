...
const createWebSocketAccept = (key: string): string => {
    return crypto.createHash('sha1').update(key + WEB_SOCKET_GUID, 'utf8').digest('base64');
};

const isWebSocketUpgradeRequest = (req: IncomingMessage): boolean => {
...
const parseNameFromPayload = (payload: string): string => {
...
const createGreetingMessage = (name: string): string => {
    return `hi ${name || 'there'}`;
};

const sendWebSocketFrame = (socket: Socket | Duplex, payload: Buffer, opcode = 0x1): void => {
...
const sendGreetingOverSocket = (socket: Socket | Duplex, rawPayload: string): void => {
...
interface WebSocketParseResult {
    consumed: number;
    opcode: number;
    payload: Buffer;
}

const tryParseWebSocketFrame = (buffer: Buffer): WebSocketParseResult | null => {
    if (buffer.length < 2) {
        return null;
    }
...
const attachWebSocketHandlers = (socket: Socket | Duplex, initialData: Buffer): void => {
    let buffered = initialData.length ? Buffer.from(initialData) : Buffer.alloc(0);

    const processBuffer = (): void => {
        let result: WebSocketParseResult | null;
...
const handleGetRoot = (_req: IncomingMessage, res: ServerResponse): void => {
    sendJson(res, 200, {
        name: NAME,
        role: 'Developer',
...
const handlePostRoot = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    try {
...
const handleDeleteRoot = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    try {
        const body = await parseRequestBody(req);
        const roll = String(body.roll ?? '');
...
const printFunction = (value: string): void => {
